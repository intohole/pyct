...
_CONDITION_TYPES = [("ALL", "^\\*$"), ("TIME_RANGE", "^(\d+)-(\d+)$"),
                    ("EVERY", "^\\*/(\d+)$"),
                    ("TIME_RANGE_EVERY", "^(\d+)-(\d+)/(\d+)$"), ("NUM",
                                                                  "^(\d+)$")]
_CONDITION_TYPE_PATTERN = re.compile(
    ur"|".join(ur"(?P<%s>%s)" % (_type, _pattern)
               for _type, _pattern in _CONDITION_TYPES))
_CONDITION_TYPE_PATTERNS = collections.OrderedDict([
    (_type, re.compile(_pattern).match) for _type, _pattern in _CONDITION_TYPES
])
...
class CTCondition(object):
    ...
    def _init_params(self, condition_type, group_tuples):
        ...
        self.judge = None
        group_tuples = [
            int(value) if value != "*" else value for value in group_tuples
        ]
    ...
    def _check_num_range(self, value):
        ...
        if value != "*" and value > _value_range:
            raise ValueError(
                "crontab time is valid , value range not right , please check %s"
    ...
    def _check_params(self):
        if self._start_time and self._end_time:
            if self._start_time >= self._end_time:
                raise ValueError(
                    "crontab time is valid ! %s" % self.condition_string)
    ...
    def __str__(self):
        return "start_time:{start_time} end_time:{end_time} every:{every} num:{num}".format(
...
class CTItem(object):
    ...
    def parse(self, condition):
        ...
        return [
            CTCondition(param, self.TIME_TYPE[index])
...
class CrontabObject(object):
    ...
    def __init__(self,
        ...
        if ct_string is None and (week is None or month is None or day is None
                                  or hour is None or minute is None):
            raise ValueError
        if ct_string is not None:
            self.week, self.month, self.day, self.hour, self.minute = self.parser(
                ct_string)
        else:
            self.week, self.month, self.day, self.hour, self.minute = CTItem(
                week), CTItem(month), CTItem(day), CTItem(hour), CTItem(minute)
...
class PyCt(threading.Thread):
    ...
    def add(self, ctstring, command, *argv, **kw):
        self.crontabs.append(
            CrontabObject(thread2.Command(command, *argv, **kw), ctstring))
    ...
    def run(self):
        ...
        while self._run_flag:
            ...
            for ct in self.crontabs:
                if ct == (_now.tm_min, _now.tm_hour, _now.tm_mday, _now.tm_mon,
                          _now.tm_wday):
